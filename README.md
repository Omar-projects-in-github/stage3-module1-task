...
- [x] Get News by id operation – return object by provided newsModels id.
- [x] For update operation – update only title, content, authorId by provided newsModels id and return updated object.
- [x] Delete News operation – delete entity from list objects by provided newsModels id and return boolean value.
...
5. All the newsModels data should be generated automatically in the module-repository with the amount of 20 and stored in array (database emulation) when the application is running.
   Reading default data for newsModels generating should be from resources package in author.txt, content.txt and newsModels.txt files.
...
The finished sample application can be found in the [__solution__](./solution/newsModels-management.jar) package at the root of the project, which can be run with the following command:
``` java
   java -jar newsModels-management.jar
```
