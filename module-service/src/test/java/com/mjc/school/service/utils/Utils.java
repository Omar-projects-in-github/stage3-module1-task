...
import static com.mjc.school.service.utils.Constant.*;
...
public class Utils {
    ...
    public static List<Class> getServicePackage() {
        return getClasses().stream()
                .filter(
                        clazz ->
                                clazz.getName().contains(MODULE_DIRECTORY_PATH)
                                        && clazz.getName().contains(SERVICE_DIRECTORY_PATH))
                .collect(Collectors.toList());
    }

    public static List<Class> getServiceImplementationPackageClasses() {
        List<Class> classes = new ArrayList<>();
        List<Class> serviceClasses = getServicePackage();
        for (Class serviceClass : serviceClasses) {
            if (isExistImplementDirectoryNameInClassPathName(serviceClass)) {
